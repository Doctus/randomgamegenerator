...
def getArtifood():
  ...
  pat = pat.replace("$numb", random.choice(numbers).capitalize())
  pat = pat.replace("$numpost", random.choice(numpost).capitalize())
  pat = pat.replace("$item", getFood().capitalize())
  return pat
...
def getName(nametype):
  ...
  typedic = {"dwarfmale":_getDwarvenMaleName,
             ...
             "proverb":_getProverb,
             "personality":_getPersonality,
             "food":getFood,
             "artifood":getArtifood,
             "kaijyuu":_getKaleidoscope}
  if nametype == "keys":
    return typedic.keys()
  if nametype == 'kaikeys':
    return ['dwarfmale', 'japanesefull', 'french', 'weapon', 'eviltitle', 'macguffin', 'food']
